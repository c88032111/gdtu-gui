...
export default {
  ...
  computed: {
    ...
    fee () {
      if (this.selectedGasPrice) {
        ...
        return this.$web3.utils.fromWei(fee, 'gdtuer')
      } else {
        return 0
      }
    },
  ...
  methods: {
    ...
    getBalance (item) {
        let _this = this
        this.$web3.gdtu.getBalance(item.address)
            .then(data => {
                // item.balance = _this.$web3.utils.fromWei(data, 'gdtuer')
                item.balance = data
            })
            .catch(err => {
                console.log(err)
    ...
    sendTransaction:function(){
      console.log("send")
      let self=this;
      let value = this.$web3.utils.toWei(this.amount.toString(), 'gdtuer');
      let account = null

      try {
        account = this.$web3.gdtu.accounts.decrypt(JSON.stringify(this.accountInfo.keystore), this.fromInfo.passworld)
      } catch (e) {
        console.log('sendDecryptWalletError', e)
        this.$message.error(this.$t('page_transfer.msg_info.decrypt_err'))
        return
      }
      //Signs an Gdtu transaction with a given private key
      this.$web3.gdtu.accounts.signTransaction({
        to: this.toAccount,
        value: value,
